  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (!result || typeof result !== "object") {
        return json(400, { ok: false, error: "missing result" });
      }
      const doc = {
        kind: kindBody,
        number: Number(result.value ?? result.number ?? result.n ?? 0),
        color: String(result.color || ""),
        round_id: result.round_id || result.roundId || result.gameId || null,
